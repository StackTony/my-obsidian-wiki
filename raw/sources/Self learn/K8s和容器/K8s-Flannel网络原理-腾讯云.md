...
flanneld是由c语言直接实现的，关键代码在/backend/udp/proxy\_adm64.c
...
- kube-flannel不会在新增节点的时候就增加arp表和fdb表，而是在数据包传递的过程中，需要目标ip的mac地址但没有找到时会发送一个l3miss的消息（RTM\_GETNEIGH）给用户态的进程，让用户进程补充邻居表信息；
