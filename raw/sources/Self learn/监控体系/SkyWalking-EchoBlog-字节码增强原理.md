---
title: SkyWalking 字节码增强原理
created: 2026-05-18
tags: [SkyWalking, 字节码增强, Java Agent]
credibility: low
source_url: https://houbb.github.io/2019/04/01/monitor-skyworking-04-why
---
...
通过agent.sample\_n\_per\_3\_secs设置3秒内采样的数量，一般500~2000是合适的值。默认-1全采样。
...
- [字节码增强](#字节码增强)
	- [字节码和Java类加载机制](#字节码和java类加载机制)
		- [运行时类的重载](#运行时类的重载)
		- [代码AOP](#代码aop)
				- [静态重写](#静态重写)
				- [动态重载](#动态重载)
		- [premain和 agentmain premain](#premain和-agentmain-premain)
		- [【premain】](#premain)
				- [agentmain premain](#agentmain-premain)
		- [字节码增强](#字节码增强-1)
- [Skywalking实现](#skywalking实现)
	- [skywalking整体架构](#skywalking整体架构)
		- [追踪实现原理](#追踪实现原理)
		- [Agent和Plugin](#agent和plugin)
		- [TraceSegment设计](#tracesegment设计)
		- [UI](#ui)
		- [TraceId设计](#traceid设计)
		- [请求采样设计](#请求采样设计)
		- [1、skywalking agent调整采样率，减少数据上传](#1skywalking-agent调整采样率减少数据上传)
				- [2、collector调整采样率，丢弃数据](#2collector调整采样率丢弃数据)
		- [数据收集和消费（轻量级队列内核）](#数据收集和消费轻量级队列内核)
		- [1、生产数据，先判断存储在哪个Buffer中，再判断存储在Buffer的哪个位置。](#1生产数据先判断存储在哪个buffer中再判断存储在buffer的哪个位置)
				- [2、消费消息，每个消费者可以有多个消费线程](#2消费消息每个消费者可以有多个消费线程)
		- [性能剖析实现原理](#性能剖析实现原理)
- [参考资料](#参考资料)