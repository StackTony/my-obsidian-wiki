...
| **Seccomp-BPF** | syscall过滤 | cBPF（同源[[concepts/linux-tracing-frameworks|eBPF]]但进化方向不同） |
...
| 功能域 | 核心组件 | 演进方向 |
|--------|----------|----------|
| **网络** | kube-proxy iptables → IPVS → Cilium [[concepts/linux-tracing-frameworks|eBPF]] | 规则规模驱动 |
...
规模增长迫使网络方案逐代升级：iptables O(n) → IPVS O(1) → Cilium [[concepts/linux-tracing-frameworks|eBPF]]（跳过整个netfilter栈）。500 Service集群就有1000-2000条iptables规则，P99延迟暴涨——规模是换代的驱动力。 ^[inferred]

### v1→v2→Rootless：安全的三步演进

Cgroup v1独立层级→v2统一层级+writeback-aware；Namespace全部特权→User Namespace使能Rootless容器；Seccomp strict→filter-BPF→[[concepts/linux-tracing-frameworks|eBPF]]生态。每一步都让容器更安全、更标准。 ^[inferred]
...
- Cilium [[concepts/linux-tracing-frameworks|eBPF]]成为K8s网络标准后的生态影响
