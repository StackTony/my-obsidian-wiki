...
| 领域 | 监控模式 | 追踪模式 | 事后分析 |
|------|----------|----------|----------|
| **CPU** | top/iostat(%ST)/kvmtop(EXT)/ps | perf record/sched/kprobe(ftrace) | perf report/火焰图 |
...
| **崩溃** | - | - | crash(vmcore) |
| **代码调试** | - | gdb attach/break | gdb(core dump) |

### 工具互补关系

**perf ↔ 火焰图**：perf record 采集 → FlameGraph 脚本可视化 → 回到 perf report 精细分析。这是最典型的"采集→可视化→精细化"三步链路。 ^[inferred]
...
**gdb ↔ crash**：gdb 用于用户态程序逐步调试（交互式），crash 用于内核 vmcore 批量分析（命令式）。调试QEMU虚拟化进程时两者可串联：gdb调试初始化流程 → 运行中perf监控 → 崩溃后crash分析。 ^[inferred]
...
- **gdb+libvirtd** → gdb attach libvirtd → follow-fork-mode child → 调试QEMU初始化
