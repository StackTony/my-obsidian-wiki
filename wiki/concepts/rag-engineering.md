...
relationships:
  ...
  - target: "[[concepts/rag-storage-technology]]"
    type: uses
  - target: "[[concepts/rag-tools-landscape]]"
    type: uses
relationships:
  - target: "[[concepts/llm-infra-landscape]]"
    type: uses
...
| m3e-base/m3e-large | 768/1024 | 国产中文专属，效果稳速度快，社区常用 |
...
- 生成层（RAGAS，详见 [[entities/ragas-framework]]）：Faithfulness、Answer Relevancy、Context Precision、Context Recall

评估数据集：自建500–2000条人工标注、开源(CRAG/MS MARCO/BEIR/T2Ranking/MultiHop-RAG)、LLM-as-Judge(GPT-4o/DeepSeek-V3)；评测基准分类见 [[concepts/llm-benchmarks]]
...
**三阶段落地**：
- **阶段一(1–2周)**：跑通流程，PyMuPDF+Chroma+BGE-M3+GPT-4o-mini+LlamaIndex，验收：回答率≥70%
- **阶段二(2–4周)**：建评估集、查询改写、混合检索+重排序、Prompt优化，目标70%→85%+
- **阶段三(4–8周)**：迁移生产向量库、全链路日志、数据飞轮、监控仪表盘、定期评估 ^[inferred]

**常见坑**：
- ❌ 过早优化：没跑通基础RAG就玩GraphRAG/Agentic RAG
- ❌ 一刀切Chunk：不同文档用同一套切分参数
- ❌ 只依赖向量检索：精确词场景必须配合BM25
- ❌ 只看最终答案不看召回质量：检索坏了LLM再强也没用
- ❌ Demo好就上生产：生产数据远比Demo脏 ^[inferred]

## 延伸阅读

相关概念：[[concepts/data-flywheel]] — 数据飞轮与RAG的数据基础循环
相关概念：[[concepts/graphrag-engineering]] — GraphRAG工程（全局型问题的解决方案）
相关概念：[[concepts/rag-storage-technology]] — RAG四层存储架构
相关概念：[[concepts/rag-tools-landscape]] — RAG核心工具全景
