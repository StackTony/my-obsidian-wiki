...
1. **消息通知机制** — 前端kick通知后端取请求，后端interrupt通知前端处理完成。通过[[summaries/virtio-io-notification-mechanism]]的ioeventfd/irqfd实现[[concepts/zero-copy-memory-mapping|零拷贝]]通知。
...
演进逻辑：数据面逐步脱离QEMU用户态，从完全软件模拟到内核态offload再到用户态DPDK轮询再到硬件[[concepts/linux-device-passthrough|设备直通]]^[inferred]。
...
- 数据面：虚拟机与网卡之间[[concepts/linux-device-passthrough|设备直通]]，类似SR-IOV直通性能
