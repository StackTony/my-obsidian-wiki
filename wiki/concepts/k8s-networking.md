...
- **iptables O(n) 是大规模集群的性能杀手**：500 Service → 1000-2000条 iptables 规则 → IPVS O(1) 或 Cilium eBPF 是出路。iptables 基于[[concepts/linux-network-stack|Linux网络协议栈]]的netfilter实现。 ^[inferred]
...
官方文档指出 kube-proxy 是可选的——当 CNI 插件提供等效的 Service 流量代理行为时。Cilium 正是如此：用 eBPF map 做 O(1) 路由查找，跳过整个[[concepts/linux-network-stack|Linux网络协议栈]]的 netfilter 栈。
